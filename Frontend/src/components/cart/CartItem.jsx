  ...
  return (
    <div className="cartItem md:flex p-4" key={cartItem.id}>
      ...
      <div className="ml-auto justify-center items-center flex max-md:flex-col">
        ...
        <div className="flex">
          <button onClick={() => removeFromCart(cartItem.id)} className="border-y-2 border-l-2 rounded-l-lg border-black">
            <img src={minusIcon} alt="Minus" className="cartIcons" />
          </button>
          <div className="amountNumber w-16 text-3xl border-2 border-black flex items-center justify-center">{cartItems[cartItem.id]}</div>
          <button onClick={() => addToCart(cartItem.id)} className="border-y-2 border-r-2 rounded-r-lg border-black">
